"""Provides the `ntnts` CLI command for Octopolars."""
...
class DefaultCommandGroup(click.Group):
    ...
    - If not, it inserts 'repos' as the subcommand, so that "ntnts lmmx"
      behaves like "ntnts repos lmmx".
    """
...
@ntnts.command(
    help="""
    NTNTS - A CLI for inventorying intents for GitHub repos or files by username, with filters.

    By default, this prints a table of repository-grounded intents.
...
@ntnts.command(help="List issues for the given GitHub username.")
...
def issues(
    ...
    """GitHub issues subcommand: 'ntnts issues <username>'."""
    show_tbl_rows = rows
    show_tbl_cols = cols
    if quiet:
        show_tbl_rows = None
